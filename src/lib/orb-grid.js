...
// Text size of the light/alert badges on a tile: proportional to the tile
// (1.2× the design's 12px on 156px), but kept readable on small phone tiles
// and from crowding the photo on large ones.
const BADGE_FONT_RATIO = (12 / 156) * 1.2;
const BADGE_FONT_MIN = 13;
const BADGE_FONT_MAX = 16;

export function orbBadgeFont(size) {
  return Math.round(clamp(BADGE_FONT_MIN, size * BADGE_FONT_RATIO, BADGE_FONT_MAX) * 100) / 100;
}
